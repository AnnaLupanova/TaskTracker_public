...
from wtforms import StringField, SearchField, BooleanField, PasswordField, SubmitField, TextAreaField
...
class AddUser(FlaskForm):
    ...
    email = StringField("Email: ", validators=[DataRequired(), Email(), Length(min=4, max=50)], render_kw={'class':'form-control '})
    first_name = StringField("First name: ", validators=[DataRequired(), Length(min=4, max=40,
                                                                    message='Name field must be at least %(min)d characters long '
                                                                            'and %(max)d at most.')],  render_kw={'class':'form-control '})
    ...
    password = PasswordField("Password: ", validators=[DataRequired(),
                                                       Regexp(r"^(?=.*?[a-z])(?=.*?[0-9]).{6,}$",
                                                              message='Length of at least 6 characters, '
                                                                      'at least one letter and at least one number')],  render_kw={'class':'form-control'})
...
class editUser(FlaskForm):
    ...
    password = StringField("Password: ", validators=[], render_kw={'class': 'form-control'}, widget=PasswordInput(hide_value=False), default="old_password_input")
    photo = FileField(validators=[FileAllowed(photos, 'Image only!')])
...
class AddProject(FlaskForm):
    ...
    name = StringField("Name: ", render_kw={'class':'form-control '}, validators=[DataRequired(), Length(min=4, max=40,
                                                                    message='Name field must be at least %(min)d characters long '
                                                                            'and %(max)d at most.')])
    description = TextAreaField("Description",render_kw={'class':'form-control '}, validators=[DataRequired(), Length(min=10, max=300,
